...
export default class ContactPage extends React.Component<PageProps> {
            ...
            {this.state.form.success ? (
              <Paragraph margin="none">
                {this.state.form.success.seconds < 60 * 60 ? (
                  <>
                    <Heading level={3}>Let's meet!</Heading>
                    <br />
                    You seem to be close to <b>{this.state.form.success.location}</b> while I am near{' '}
                    <b>{this.state.form.success.mylocation}</b> - which is about{' '}
                    <b>
                      {this.state.form.success.traveltime} ({this.state.form.success.distance})
                    </b>
                    away from each other. <a href={this.state.form.success.url}>Show me on Google Maps!</a>
                    <br />
                  </>
                ) : (
                  <>
                    <Heading level={3}>Reach out!</Heading>
                    <br />
                    It looks like it'd take longer than 1 hour ({this.state.form.success.traveltime} or {this.state.form.success.distance}{' '}
                    to be exact) to meet up.
              ...
              <Form errors={this.state.form.error} onSubmit={this.send}>
                ...
                <p>
                  <Heading level={3}>What will happen on pressing submit?</Heading>
                  <br />
                  <ul>
                    <li>
                      Your browser will ask you to allow the site to read <b>your geoposition</b>.
                    </li>
                    <li>
                      Afterwards the browser will load <b>my geoposition</b>.
                    </li>
                    <li>
                      Both will be send to the <b>google-maps api</b> to calculate the best way to get to each other.
                    </li>
                    <li>
                      All this data together with the data you entered will be <b>storred in a spreadsheet in my google drive</b> and
                    </li>
                    <li>
                      <b>I am notified</b> on my phone that the spreadsheet was edited (it might take some minutes until the change is
                      picked up).
                    </li>
                    <li>
                      <b>You are shown the distance and time</b> between us together with <b>a link to start the navigation</b> using google
                      maps.
                    </li>
                  </ul>
                  With our positions and distance been exchanged, we can go forward and decide if we want to meet up. The data you enter
                  will only be used for this purpose and I will delete the spread-sheet entry as we've clarified how to move forward.
                  <br />
                  As per Directions API Policies:
                  <br />
                  This site currently does not have Terms of Use or a Privacy Policy. Nevertheless be made aware that:
                  <br />
                  Using this application application (the meetup functionality), users (you) are bound by{' '}
                  <a href="https://www.google.com/intl/en/policies/terms">Google’s Terms of Service</a>.<br />
                  This application (the meetup functionality) uses Google Maps APIs and which adhere to{' '}
  ...
  private send() {
    ...
    const myLocation = fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${sheet}?key=${key}&fields=sheets.data.rowData.values.effectiveValue`,
    )
      .then(i => i.json())
    ...
    Promise.all([getGeolocation, myLocation, loadMaps()])
      ...
      .then(([geolocation, myloc, directions]) =>
        Promise.all([
          fetch('https://hooks.zapier.com/hooks/catch/2584025/a9wh9p/', {
            body: JSON.stringify({
              name: form.name,
